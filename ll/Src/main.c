...
const osMessageQueueAttr_t monQueue_attributes = { .name = "monQueue" };
...
int main(void) {
	...
	tempQueueHandle = osMessageQueueNew(16, sizeof(SensorMessage_t),
			&tempQueue_attributes);

	/* creation of monQueue */
	monQueueHandle = osMessageQueueNew(16, sizeof(SensorMessage_t),
			&monQueue_attributes);
...
void CommTask(void *argument) {
	...
	SensorMessage_t rcvMsg;
	char buffer[60];

	for (;;) {
		// Wait for message from queue
		if (osMessageQueueGet(tempQueueHandle, &rcvMsg, NULL, osWaitForever)
				== osOK) {
			// Format output string
			int len = sprintf(buffer, "Temp: %d C | Time: %lu ms\r\n",
					rcvMsg.temperature, (unsigned long) rcvMsg.timestamp);
...
void MonitorTask(void *argument) {
	...
	for (;;) {
		if (osMessageQueueGet(monQueueHandle, &msg, NULL, osWaitForever)
				== osOK) {
